...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \begin{scope}
    ...
    \begin{scope}[shift={($(current page.south west)+(-1.5,4.3)$)},rotate=-5]
      % cuerpo del mástil
      \draw[lightblue,opacity=0.35,line width=1pt] (0,0) rectangle (25,4.2);
      % trastes
      \foreach \x in {0,1.9,3.8,5.9,8.1,10.5,13.1,15.9,18.9,22.1,25}{
        \draw[white,opacity=0.22,line width=0.5pt] (\x,0) -- (\x,4.2);
      }
      % cuerdas
      \foreach \y in {0.35,1.05,1.75,2.45,3.15,3.85}{
        \draw[white,opacity=0.30,line width=0.35pt] (0,\y) -- (25,\y);
      }
      % marcadores de nota
      \filldraw[lightblue] (4.8,2.1) circle (0.16);
      \filldraw[amber]     (9.3,1.05) circle (0.16);
      \filldraw[white]     (14.5,3.15) circle (0.16);
      \filldraw[amber]     (17.4,2.45) circle (0.16);
      \filldraw[lightblue] (21.5,0.7) circle (0.16);
      \filldraw[white]     (23.8,2.8) circle (0.13);
    \end{scope}
  \end{scope}
...
Otras devDependencies & \lstinline!postcss!, \lstinline!autoprefixer!, \lstinline!puppeteer-core! (usado en \lstinline!scripts/visualReview*.mjs!) & ---\\
\end{longtable}
\end{center}
...
\begin{hallazgobox}{Recuento de tests}
El repositorio contiene \textbf{44} ficheros \lstinline!*.test.*! reales bajo \lstinline!src/! / \lstinline!scripts/! (confirmado por \lstinline!find!), pero \lstinline!vitest run! reporta \textbf{60} ficheros ejecutados. La diferencia (16 ficheros) proviene de copias de tests alojadas en directorios auxiliares de trabajo ajenos al árbol fuente de la aplicación (dos directorios, cada uno con una copia parcial de 8 ficheros de test de \lstinline!src/!), que \textbf{no están excluidos} por \lstinline!vite.config.js:6-8! (\lstinline!test.exclude! solo cubre \lstinline!e2e/**! y \lstinline!node_modules/**!). No afecta a la corrección del resultado (son copias idénticas que también pasan), pero infla el recuento reportado y ejecuta trabajo redundante. Ver recomendación en \Sref{sec:18}.
\end{hallazgobox}
...
\begin{hallazgobox}{8 --- Recuento de tests inflado por directorios auxiliares de trabajo no excluidos}
\lstinline!vitest run! ejecuta 60 ficheros en vez de los 44 reales del proyecto porque copias de test alojadas en directorios auxiliares ajenos al árbol fuente de la aplicación (con 8 tests cada uno) no están excluidas en \lstinline!vite.config.js! (\lstinline!test.exclude! solo cubre \lstinline!e2e/**! / \lstinline!node_modules/**!). No afecta a la corrección, pero es ruido operativo y trabajo redundante en cada \lstinline!npm test!.
\end{hallazgobox}
...
\begin{recobox}{1 --- Excluir los directorios auxiliares de trabajo ajenos al árbol fuente}
De \lstinline!vite.config.js: test.exclude! (junto a \lstinline!e2e/**! / \lstinline!node_modules/**!). Corrige de inmediato el recuento inflado de tests (\Sref{sec:15.2}) y evita ejecutar trabajo redundante en cada \lstinline!npm test!; es un cambio de una línea sin riesgo, siempre que se identifiquen dichos directorios en cada entorno de trabajo.
\end{recobox}
