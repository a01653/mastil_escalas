...
export default function FretboardScalesPage() {
  ...
  // Booleano derivado de nearSlots: ¿el slot activo prefiere cuatriadas?
  // Es el único dato de nearSlots que necesita useHarmonyFeature.
  const harmonyWithSeventh = useMemo(() => {
    const base = nearSlots.find((s) => s?.enabled && String(s?.family || "tertian") === "tertian")
      || nearSlots.find((s) => !!s?.enabled)
      || nearSlots?.[0]
      || {};
    return String(base?.family || "tertian") === "tertian"
      ? base.structure === "tetrad" || !!base.ext7
      : true;
  }, [nearSlots]);

  const harmony = useHarmonyFeature({
    scaleTetradHarmony,
    rootPc,
    scaleName,
    scaleIntervals,
    ...
    preferSharps,
    spelledScaleNotes,
    withSeventh: harmonyWithSeventh,
  });
  const { scaleTetradDegreesText, scaleTetradNotesText, harmonizedScale } = harmony;
    ...
    return (
      <div className={paneClassName}>
        ...
        <NearChordsPanel
          ...
          nearSlots={nearSlots}
          nearComputed={nearComputed}
          nearWindowStart={nearWindowStart}
          nearWindowSize={nearWindowSize}
          nearWindowSizeRaw={nearWindowSizeRaw}
          nearAutoScaleSync={nearAutoScaleSync}
          nearBgColors={nearBgColors}
          mobileNearChordEditorIdx={mobileNearChordEditorIdx}
          nearFrom={nearFrom}
          nearTo={nearTo}
          nearStartMax={nearStartMax}
          nearTonalityAnalysis={nearTonalityAnalysis}
          ...
          setNearWindowStart={setNearWindowStart}
          ...
          buildNearSlotNoteMeta={buildNearSlotNoteMeta}
          buildNearSlotStudyEntry={buildNearSlotStudyEntry}
        />
